...
Follow the six-step procedure in `skills/verify/references/procedure.md`. Execute each step, track pass/fail, and stop to troubleshoot if any step fails.
...
If any step failed, suggest running `/credyt:setup` to correct the configuration, then `/credyt:verify` again.
